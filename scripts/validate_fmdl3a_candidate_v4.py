...
def main() -> int:
    ...
    if missing:
        payload = {"validation_version": "1.3.0", "program_id": "FMDL-3A", "status": "FAIL", "checks": checks, "hard_failures": failures}
    ...
    trade_none = config.get("trade_authority") == "NONE" and decision.get("trade_authority") == "NONE" and trade_values <= {"NONE"} and source_trade_values <= {"NONE"} and support_trade_values <= {"NONE"}
    add_check(checks, failures, "ZERO_TRADE_AUTHORITY", trade_none, f"rows={trade_values}; source_index={source_trade_values}; support={support_trade_values}")
    ...
    capital_rows = rows[(rows["source_id"] == "EASTMONEY_INDIVIDUAL_INFO") & (rows["component"] == "CURRENT_PRICE_MARKET_CAP_AND_SHARE_COUNT") & (rows["symbol"].isin(supported_symbols))]
    successful_capital = set(capital_rows.loc[capital_rows["status"] == "SUCCESS", "symbol"].astype(str))
    capital_coverage = len(successful_capital) / len(supported_symbols) if supported_symbols else 0.0
    add_check(checks, failures, "SUPPORTED_UNIVERSE_CURRENT_CAPITALIZATION_GATE", capital_coverage >= policy["minimum_supported_universe_current_capitalization_coverage"], f"coverage={capital_coverage:.6f}; missing={sorted(supported_symbols - successful_capital)}")
    ...
    route_expectations = {
        ...
        "EASTMONEY_INDIVIDUAL_INFO": "PRIMARY_CURRENT_PRICE_MARKET_CAP_AND_SHARE_COUNT_SOURCE_SUPPORTED_UNIVERSE",
