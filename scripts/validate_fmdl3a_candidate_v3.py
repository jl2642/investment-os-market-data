...
def main() -> int:
    ...
    trade_none = (
        config.get("trade_authority") == "NONE"
        and decision.get("trade_authority") == "NONE"
        and trade_values <= {"NONE"}
        and source_trade_values <= {"NONE"}
        and support_trade_values <= {"NONE"}
    )
    add_check(
        checks,
        failures,
        ...
        trade_none,
        f"rows={trade_values}; source_index={source_trade_values}; support={support_trade_values}",
    )
    ...
    bse_controlled = bool(
        len(bse)
        and bse["statement_status"].eq("QUARANTINED").all()
        and bse["official_document_source_available"].astype(str).str.lower().isin({"true", "1"}).all()
        and bse["status_reason"].astype(str).str.contains("CNINFO", na=False).all()
    )
    ...
    xueqiu_bundle = rows[(rows["source_id"] == "XUEQIU_CURRENT_VALUATION") & (rows["component"] == "SAMPLE_CURRENT_VALUATION_BUNDLE")]
    valuation_coverage = float(xueqiu_bundle["sample_value_coverage_ratio"].iloc[0]) if len(xueqiu_bundle) else 0.0
    add_check(checks, failures, "XUEQIU_CURRENT_VALUATION_GATE", valuation_coverage >= policy["minimum_current_valuation_sample_coverage"], f"coverage={valuation_coverage:.6f}")
    ...
    route_expectations = {
        ...
        "XUEQIU_CURRENT_VALUATION": "PRIMARY_CURRENT_MARKET_CAP_PE_PB_SHARE_COUNT_SOURCE",
    ...
    measured_gates_match = decision.get("frozen_numeric_gates") == policy
