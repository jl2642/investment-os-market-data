...
def main():
    ...
    fail(e, market.get('status')=='READY' and '2026-08-07' in json.dumps(market), 'A_MARKET')
    fail(e, history.get('status')=='READY' and '2026-08-07' in json.dumps(history), 'A_HISTORY')
    fail(e, factors.get('status')=='READY' and '2026-08-07' in json.dumps(factors), 'A_FACTORS')
    fail(e, bool(screening.get('parent_pass',False)), 'A_SCREENING')
    ...
    fail(e, loop.get('status')=='ROUND2_OPERATING_OBSERVATION' and loop.get('completed_weekly_cycle_count',0)>=1 and loop.get('admission_count')==0 and loop.get('dynamic_exit_count')==0, 'A_DYNAMIC_LOOP')
    ...
    round3=(ROOT/c['authoritative_inputs']['round3_workflow']).read_text(encoding='utf-8'); fail(e, 'sec_mode' in round3 and 'workflow_dispatch' in round3 and 'us' in round3.lower(), 'US_BOUNDED_WORKFLOW')
