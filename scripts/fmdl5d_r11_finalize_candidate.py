...
def main() -> int:
    ...
    if disclosure.get("program_id") != "FMDL-5D-R1.1":
        raise ValueError(f"R11_DISCLOSURE_STATUS_MISSING:{disclosure.get('program_id')}")
    if disclosure.get("runtime_failures"):
        raise ValueError(f"R11_DISCLOSURE_RUNTIME_FAILURES:{disclosure['runtime_failures']}")

    runtime["program_id"] = "FMDL-5D-R1.1"
    runtime["repair_round"] = "FMDL-5D-R1.1"
    runtime["runtime_orchestration"] = "DUAL_SHARDED_CHECKPOINTED_AGGREGATION"
    write_json(candidate / "FMDL5D_R1_RUNTIME_REPORT.json", runtime)

    limitations = list(quality.get("controlled_limitations", []))
    limitations = [item for item in limitations if "FMDL-5D-R1 executes disclosure" not in item]
    limitations.append(
        "FMDL-5D-R1.1 executes HKEXnews disclosure months and structured-financial securities as independent checkpoint shards before deterministic aggregation."
    )
    quality["controlled_limitations"] = limitations
    quality["r11_disclosure_expected_shard_count"] = disclosure.get("expected_shard_count")
    quality["r11_disclosure_completed_shard_count"] = disclosure.get("completed_shard_count")
    quality["r11_disclosure_expected_monthly_chunk_count"] = disclosure.get("expected_monthly_chunk_count")
    quality["r11_disclosure_completed_monthly_chunk_count"] = disclosure.get("completed_monthly_chunk_count")
    write_json(candidate / "FMDL5D_QUALITY_REPORT.json", quality)

    source_registry["runtime_orchestration"] = "FMDL-5D-R1.1_DUAL_SHARDED_CHECKPOINTED_AGGREGATION"
    source_registry["disclosure_checkpoint_summary"] = {
        "expected_shard_count": disclosure.get("expected_shard_count"),
        "completed_shard_count": disclosure.get("completed_shard_count"),
        "expected_monthly_chunk_count": disclosure.get("expected_monthly_chunk_count"),
        "completed_monthly_chunk_count": disclosure.get("completed_monthly_chunk_count"),
    ...
    canonical_sha256 = stable_hash(
        {
            ...
            "runtime_orchestration": "R1.1_DUAL_SHARDED_CHECKPOINTED",
        }
    )
    ...
    decision["repair_round"] = "FMDL-5D-R1.1"
