...
def main() -> int:
    ...
    pointers = [
        (cfg["entry_gates"]["valuation_contract_pointer"], cfg["entry_gates"]["valuation_contract_status"]),
        (cfg["entry_gates"]["capitalization_pointer"], cfg["entry_gates"]["capitalization_status"]),
        (cfg["entry_gates"]["valuation_pointer"], cfg["entry_gates"]["valuation_status"]),
    ]
    ...
    registry = pd.read_csv(ROOT / cfg["inputs"]["event_registry"], encoding="utf-8-sig")
    if len(registry) != 8 or registry["event_type"].duplicated().any():
        errors.append("EVENT_REGISTRY_NOT_EXACT_8")
    if cfg["source"]["dividend_source_adapter"] != "akshare.stock_fhps_detail_em":
    ...
    if cfg["share_change_policy"]["unclassified_share_changes_enter_shareholder_yield"]:
