...
### Phase 3 — Historical Replay & Calibration — IN PROGRESS / R2 HOLDOUT ACCEPTED / 3D-R2 ROUND 1 COMPLETE / PHASE 4 BLOCKED
...
#### Governed Post-3F R2 Loopback — B/C + INDEPENDENT HOLDOUT COMPLETE / PR #311 → #312 → #316 → #318 → #325
...
#### Phase 3D-R2 Round 1 — MEASURABILITY CONTRACT + EVIDENCE AUDIT COMPLETE / PR #326
Round 1 froze the R2-specific measurement boundary **before any R2 realized-outcome value was read**. The primary structural evaluation unit is a checkpoint-local exact-signature dominance edge. A dominance edge is not a hypothetical trade; a Pareto frontier is not a target portfolio; R2 still creates no scalar score, global winner, target weights or trading authority.

Frozen observation convention:
- exchange trading sessions strictly after each checkpoint: **1 / 3 / 5**, with **5 sessions primary**;
- allowed later evidence is descriptive local-price-return / edge-return-spread / directional-concordance evidence only if coverage is sufficient;
- synthetic trade returns, portfolio returns, target-weight returns, Sharpe, alpha, max drawdown, probability calibration, unsupported regret, scalar ranking and winner claims remain forbidden.

The accepted outcome-blind Round 1 audit binds the Holdout denominators at **14 checkpoints / 13 comparable checkpoints / 4 distinct comparable signatures / 27 comparable groups / 85 comparable profiles / 54 dominance edges / 7 edge-endpoint securities**. Audit errors = 0; outcome-value reads = 0; future-return computations = 0; performance computations = 0.

A new R2-specific `PHASE3D_R2_OUTCOME_SOURCE_MANIFEST.json` is required. The historical Phase 3D manifest is not R2 authority. Before observing R2 outcomes, the primary 5-session evidence gate was frozen at >=80% measurable dominance edges, >=80% measurable comparable groups, >=75% measurable comparable checkpoints, and measurable edges in >=2 exact signatures. Failure of that gate is nonmeasurability/insufficient evidence, not economic underperformance.

Round 1 classification:
`PASS_R2_MEASURABILITY_CONTRACT_FROZEN_EVIDENCE_ACQUISITION_REQUIRED`.

Therefore Phase 3D-R2 has **started but is not complete**. Outcome-evidence acquisition is authorized under the frozen contract but has not started; performance measurement remains unauthorized until the coverage gate passes. Phase 3E-R2 and repeat Phase 3F remain downstream; Phase 4 remains blocked.
