...
R3_BUILDER = Path("automation/r3_position_action_matrix/build_r3_position_action_matrix.py")
R3_LINEAGE = Path("automation/r3_position_action_matrix/patch_forward_lineage.py")
...
def main() -> int:
    ...
    run("python", str(R3_BUILDER), "--repo-root", ".", "--source-head-sha", source_head)
    run("python", str(R3_LINEAGE))
    ...
    if staged.returncode == 0:
        return 0
    run("git", "commit", "-m", "R3 materialize position action matrix and user decision pack")
