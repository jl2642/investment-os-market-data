...
def account_summary(
    *,
    ...
    total_assets = position_market_value + execution_cash
    comparison = None
    ...
    summary: dict[str, Any] = {
        ...
        "reconciliation_status": (
            "CURRENT_AUTOMATED_MARK_WATERMARK_NOT_DIRECTLY_COMPARABLE_TO_HISTORICAL_WP2_3"
        ...
        "reconciliation_explanation": (
            ...
            "Historical WP2-3 used its own accepted mixed watermark. Differences are disclosed and "
            "must not be forced through position, cost or cash mutation."
        ),
    }
...
def main() -> None:
    ...
    real_cash = num(real_source.get("summary", {}).get("brokerage_available_cash"))
    sim_cash = num(sim_source.get("summary", {}).get("available_cash"))
    sim_source_total_assets = num(sim_source.get("summary", {}).get("total_assets"))
    sim_source_total_pnl = num(sim_source.get("summary", {}).get("total_pnl"))
