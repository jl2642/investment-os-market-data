...
def _controlled_inbox(queue: dict) -> dict:
    return {
...
def test_controlled_external_sec_inbox_completes_round3_without_legacy_web_claim(tmp_path: Path) -> None:
    ...
    write_json(inbox_path, _controlled_inbox(queue))
...
def test_direct_script_entrypoint_accepts_controlled_external_inbox(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [
            sys.executable,
            str(ROOT / "automation/cross_market/apply_round3_sec_observer_results.py"),
            "--repo-root",
            str(tmp_path),
            "--inbox",
            str(inbox_path.relative_to(tmp_path)),
            "--policy",
