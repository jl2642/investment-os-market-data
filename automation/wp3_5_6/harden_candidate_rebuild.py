...
def main():
    ...
    all_core_codes=sorted(str(x["security_code"]).zfill(6) for x in core)
    ...
    state["qualitative_core_retain_codes"]=all_core_codes
    ...
    manifest["qualitative_core_retain_codes"]=all_core_codes
    ...
    reg["wp3_5_6"].update({"candidate_core_proposed":len(core),"shadow_track_proposed":len(shadow),"ready_for_user_decision_proposed":len(ready),"research_queue_proposed":len(queue),"complete_entry_baselines":m["complete_entry_baselines"],"qualitative_core_retain_codes":all_core_codes})
    wj(regp,reg)
    ...
    print(json.dumps({"status":"PASS","qualitative_core_retain_codes":all_core_codes,"newly_promoted_codes":sorted(promoted),"candidate_core":len(core),"shadow":len(shadow),"ready":len(ready)},ensure_ascii=False))
