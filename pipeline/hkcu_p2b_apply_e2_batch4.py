...
COLLECTED = {"EVIDENCE_PARTIAL", "EVIDENCE_COMPLETE"}
...
def rebuild_batch3(root: Path, out: Path) -> None:
    ...
    subprocess.run([
        sys.executable, str(root / "pipeline/hkcu_p2b_apply_e2_batch3.py"),
        "--repo-root", str(root), "--output", str(out),
    ], check=True)
    subprocess.run([
        sys.executable, str(root / "scripts/validate_hkcu_p2b_e2_batch3.py"),
        "--output", str(out),
    ], check=True)
    ...
    for _, sec in wide.iterrows():
        for dimension, prefix in DIMS.items():
            ...
            rows.append({
                ...
                "evidence_count": 1 if status in COLLECTED else 0,
                "score": pd.NA,
...
def build(root: Path, out: Path) -> None:
    contract_path = root / "config/hkcu_p2b_e2_batch4_contract.json"
    ...
    if len(batch4) != int(policy["required_evidence_rows"]):
        failures.append(f"BATCH4_EVIDENCE_ROWS:{len(batch4)}")
    if batch4.duplicated(["security_id", "research_dimension"]).any():
        failures.append("BATCH4_DUPLICATE_SECURITY_DIMENSION")
    if set(batch4["research_dimension"]) != set(policy["required_dimensions"]):
        failures.append("BATCH4_DIMENSION_SET_INVALID")
    if not set(batch4["evidence_status"]).issubset(set(contract["evidence_policy"]["allowed_statuses"])):
        failures.append("BATCH4_STATUS_VOCABULARY_INVALID")
    if batch4["score"].notna().any():
        failures.append("BATCH4_ALPHA_SCORE_PRESENT")

    as_of = pd.Timestamp(policy["as_of_date"])
    for _, row in batch4.iterrows():
        ...
        if row["evidence_status"] in COLLECTED:
    ...
    batch_counts = batch4["evidence_status"].value_counts().to_dict()
    ...
    collected4 = int(batch4["evidence_status"].isin(COLLECTED).sum())
    if collected4 != int(expected_batch["evidence_collected_rows"]):
        failures.append(f"BATCH_COLLECTED_COUNT:{collected4}")

    direct = set(
        batch4.loc[
            (batch4["research_dimension"] == "EARNINGS_EXPECTATION_REVISION")
            & (batch4["evidence_status"] == "EVIDENCE_COMPLETE"),
            "stock_code_5d",
    ...
    b3_ledger = pd.read_csv(
        b3 / "HKCU_P2B_E2_B3_CUMULATIVE_EVIDENCE_LEDGER.csv", dtype={"stock_code_5d": str}
    )
    dim = pd.read_csv(
        b3 / "HKCU_P2B_E2_B3_DIMENSION_MATRIX.csv", dtype={"stock_code_5d": str}
    )
    ...
    batch4_keys = set(zip(batch4["security_id"], batch4["research_dimension"]))
    if batch4_keys - company_keys:
        failures.append("BATCH4_EVIDENCE_NOT_IN_COMPANY_MATRIX")
    ...
    for _, row in batch4.iterrows():
        ...
        if len(prior) != 1 or str(prior.iloc[0]["evidence_status"]) != "RESEARCH_REQUIRED":
            failures.append(f"BATCH4_TARGET_NOT_UNSTARTED:{row['security_id']}:{row['research_dimension']}")

    idx_map = {(sid, rd): idx for idx, sid, rd in zip(dim.index, dim["security_id"], dim["research_dimension"])}
    for _, ev in batch4.iterrows():
        ...
        dim.at[idx, "score_status"] = "EVIDENCE_COLLECTED_NO_ALPHA_SCORE" if status in COLLECTED else "NO_SCORE_BEFORE_EVIDENCE"
    ...
    cumulative_ledger = pd.concat([b3_ledger, batch4], ignore_index=True)
    ...
    expected_cum = contract["expected_cumulative_after_batch4"]
    ...
    cumulative_collected = int(cumulative_ledger["evidence_status"].isin(COLLECTED).sum())
    ...
    out.mkdir(parents=True, exist_ok=True)
    batch_path = out / "HKCU_P2B_E2_B4_EVIDENCE_LEDGER.csv"
    ledger_path = out / "HKCU_P2B_E2_B4_CUMULATIVE_EVIDENCE_LEDGER.csv"
    dim_path = out / "HKCU_P2B_E2_B4_DIMENSION_MATRIX.csv"
    open_path = out / "HKCU_P2B_E2_B4_OPEN_RESEARCH_QUEUE.csv"
    unstarted_path = out / "HKCU_P2B_E2_B4_UNSTARTED_QUEUE.csv"
    decision_path = out / "HKCU_P2B_E2_B4_DECISION.json"
    quality_path = out / "HKCU_P2B_E2_B4_QUALITY_REPORT.json"
    batch4.to_csv(batch_path, index=False)
    ...
    decision = {
        "program_id": "HKCU-P2B-E2-B4",
        "phase": "P2B_E2_COMPANY_SPECIFIC_EVIDENCE",
        "status": "PASS_P2B_E2_RANKS_61_77_BATCH" if not failures else "FAIL_P2B_E2_BATCH4",
        "accepted_longlist_count": 77,
        "batch_rank_start": 61,
        "batch_rank_end": 77,
        "batch_security_count": int(wide["security_id"].nunique()),
        "batch_evidence_rows": int(len(batch4)),
        "batch_evidence_collected_rows": collected4,
        ...
        "first_pass_company_coverage_complete": started == 77,
    ...
    quality = {
        ...
        "batch4_status_counts": {str(k): int(v) for k, v in batch4["evidence_status"].value_counts().items()},
    ...
    manifest = {
        ...
        "inputs": {
            ...
            "upstream_batch3_ledger_sha256": sha256_file(b3 / "HKCU_P2B_E2_B3_CUMULATIVE_EVIDENCE_LEDGER.csv"),
        },
        "outputs": {p.name: sha256_file(p) for p in [batch_path, ledger_path, dim_path, open_path, unstarted_path, decision_path, quality_path]},
        "trade_authority": TRADE_AUTHORITY,
    }
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    if failures:
        raise SystemExit("P2B_E2_BATCH4_FAILED:" + "|".join(sorted(set(failures))))
