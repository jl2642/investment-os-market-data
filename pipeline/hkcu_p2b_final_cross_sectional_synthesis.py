...
def retry(fn: Callable[[], Any], attempts: int = 2, delay: float = 1.0):
    last = None
...
def extract_exact_close(df: pd.DataFrame, date_iso: str, provider: str) -> float:
    if df is None or df.empty:
        raise RuntimeError(f"{provider}:EMPTY")
    ...
    if date_col is None or close_col is None:
        raise RuntimeError(f"{provider}:SCHEMA:{list(df.columns)}")
    dates = pd.to_datetime(df[date_col], errors="coerce").dt.normalize()
    rows = df.loc[dates.eq(pd.Timestamp(date_iso).normalize())]
    if len(rows) != 1:
        raise RuntimeError(f"{provider}:DATE_MISMATCH:{date_iso}:{len(rows)}")
    value = pd.to_numeric(rows.iloc[0][close_col], errors="coerce")
    if pd.isna(value) or float(value) <= 0:
        raise RuntimeError(f"{provider}:INVALID_CLOSE:{value}")
    return float(value)


def get_h_close(code: str, date_iso: str) -> tuple[float, str]:
    code = str(code).zfill(5)
    compact = date_iso.replace("-", "")
    year = date_iso[:4]
    providers: list[tuple[str, Callable[[], pd.DataFrame]]] = [
        ("AKSHARE_STOCK_HK_DAILY_SINA", lambda: ak.stock_hk_daily(symbol=code, adjust="")),
        ("AKSHARE_STOCK_ZH_AH_DAILY_TENCENT", lambda: ak.stock_zh_ah_daily(symbol=code, start_year=year, end_year=year, adjust="")),
        ("AKSHARE_STOCK_HK_HIST_EASTMONEY", lambda: ak.stock_hk_hist(symbol=code, period="daily", start_date=compact, end_date=compact, adjust="")),
    ]
    errors: list[str] = []
    for provider, fetcher in providers:
        try:
            return extract_exact_close(retry(fetcher), date_iso, provider), provider
        except Exception as exc:
            errors.append(f"{provider}={exc}")
    raise RuntimeError(f"HK_PRICE_ALL_PROVIDERS_FAILED:{code}:{date_iso}:" + " || ".join(errors))


def get_cny_per_hkd(date_iso: str) -> float:
    df = retry(ak.currency_boc_safe, attempts=4, delay=2.0)
    if df is None or df.empty:
...
def build_ah_relative_value(root: Path, contract: dict[str, Any], p2a: pd.DataFrame) -> pd.DataFrame:
    ...
    for r in ah.itertuples(index=False):
        ...
        hc, h_provider = get_h_close(r.h_code, date_iso)
        ...
        rows.append({
            ...
            "h_price_source": h_provider,
            "h_price_source_policy": policy["ah_h_price_source"],
...
def build(root: Path, out: Path) -> dict[str, Any]:
    ...
    e1_tx = e1_dim[e1_dim["research_dimension"].eq("TRANSACTION_COST_TAX")][["security_id", "evidence_status", "evidence_count"]].copy().rename(columns={"evidence_status": "transaction_tax_evidence_status", "evidence_count": "transaction_tax_evidence_count"})
    ...
    for stage, dname, sname in specs:
        ...
        d["decision_stage"] = stage.upper(); s["decision_stage"] = stage.upper()
        dim_parts.append(d); sec_parts.append(s)
    ...
    if len(e1_tx) != int(exp["transaction_tax_complete_count"]) or not e1_tx["transaction_tax_evidence_status"].eq("EVIDENCE_COMPLETE").all(): failures.append("TRANSACTION_TAX_NOT_COMPLETE")
    ...
    counts = counts[["POSITIVE", "NEGATIVE", "MIXED", "NEUTRAL_OR_UNKNOWN"]].reset_index().rename(columns={"POSITIVE": "positive_dimension_count", "NEGATIVE": "negative_dimension_count", "MIXED": "mixed_dimension_count", "NEUTRAL_OR_UNKNOWN": "neutral_unknown_dimension_count"})
    ...
    sec = p2a[base_cols].copy().merge(counts, on="security_id", how="left", validate="one_to_one").merge(e1_tx, on="security_id", how="left", validate="one_to_one").merge(ah_status, on="security_id", how="left", validate="one_to_one")
    ...
    sec["p2a_rank_preserved_not_rescored"] = True; sec["alpha_score"] = pd.NA; sec["formal_candidate_graduation_allowed"] = False; sec["trade_authority"] = TRADE_AUTHORITY
    ...
    if not set(ah_rel["h_price_source"]).issubset(set(HK_PRICE_PROVIDERS)): failures.append("AH_H_PRICE_SOURCE")
    sec["h_discount_to_a_pct"] = sec["security_id"].map(ah_rel.set_index("security_id")["h_discount_to_a_pct"].to_dict())
    sec["ah_relative_value_direction"] = sec["security_id"].map(ah_rel.set_index("security_id")["relative_value_direction"].to_dict()).fillna("NOT_APPLICABLE")
    advance_count = int((~sec["retained_blocker"]).sum()); blocked_count = int(sec["retained_blocker"].sum())
    ...
    dim["alpha_score"] = pd.NA; dim["formal_candidate_graduation_allowed"] = False; dim["trade_authority"] = TRADE_AUTHORITY
    sec_path = out / "HKCU_P2B_FINAL_SECURITY_CROSS_SECTION.csv"; dim_path = out / "HKCU_P2B_FINAL_COMPANY_DIMENSION_SURFACE.csv"; blocker_path = out / "HKCU_P2B_FINAL_RETAINED_BLOCKERS.csv"; ah_path = out / "HKCU_P2B_FINAL_AH_RELATIVE_VALUE.csv"
    sec.to_csv(sec_path, index=False); dim.to_csv(dim_path, index=False); blocker_surface.to_csv(blocker_path, index=False); ah_rel.to_csv(ah_path, index=False)
    ...
    quality = {"program_id": PROGRAM_ID, "status": "PASS" if not failures else "FAIL", "hard_failures": sorted(set(failures)), "all_77_security_decision_surfaces_present": len(sec) == 77, "all_231_company_dimensions_present": len(dim) == 231, "p2a_rank_preserved_not_rescored": True, "p2a_security_name_schema_normalized": True, "a_snapshot_exchange_suffix_mapping": A_SNAPSHOT_SUFFIX_BY_EXCHANGE, "hk_price_provider_failover": list(HK_PRICE_PROVIDERS), "evidence_balance_descriptive_not_scored": True, "cross_dimension_event_deduplication_preserved": True, "missing_consensus_is_not_bearish": True, "transaction_tax_is_execution_context_not_alpha": True, "ah_relative_value_is_context_not_alpha": True, "ah_price_fx_synchronized_date": contract["cross_section_policy"]["ah_price_date"], "formal_candidate_graduation_allowed": False, "trade_authority": TRADE_AUTHORITY}
    decision_path = out / "HKCU_P2B_FINAL_DECISION.json"; quality_path = out / "HKCU_P2B_FINAL_QUALITY_REPORT.json"
    write_json(decision_path, decision); write_json(quality_path, quality)
    report_path = out / "HKCU_P2B_FINAL_CROSS_SECTIONAL_SYNTHESIS.md"
    report_path.write_text("\n".join(["# HKCU P2B Final Cross-sectional Synthesis", "", f"Status: **{decision['status']}**", "", f"- Securities: {len(sec)}", f"- Company decision dimensions: {len(dim)}", f"- Ready for P3 contract evaluation with confidence cap: {advance_count}", f"- Retained investment blockers: {blocked_count}", f"- Synchronized A/H relative-value observations: {len(ah_rel)} / {int(exp['true_ah_pair_count'])}", "- New alpha score: 0", "- Formal HK Candidate graduation: not allowed", "", "## Boundary", "", "This gate is a cross-sectional research-readiness surface. It preserves P2A screening rank and evidence semantics, but does not promote any security to the formal HK Candidate Pool and does not modify portfolios or create orders.", ""]), encoding="utf-8")
    ...
    for stage, p in [("P2A", upstream["p2a"] / "HKCU_P2A_DECISION.json"), ("E1", upstream["e1"] / "HKCU_P2B_E1_DECISION.json"), ("S1", upstream["s1"] / "HKCU_P2B_E2_S1_DECISION.json"), ("S2", upstream["s2"] / "HKCU_P2B_E2_S2_RANKS21_40_DECISION.json"), ("S3", upstream["s3"] / "HKCU_P2B_E2_S3_RANKS41_60_DECISION.json"), ("S4", upstream["s4"] / "HKCU_P2B_E2_S4_RANKS61_77_DECISION.json")]: manifest["upstream_decision_sha256"][stage] = sha256_file(p)
    for p in [sec_path, dim_path, blocker_path, ah_path, decision_path, quality_path, report_path]: manifest["files"][p.name] = {"sha256": sha256_file(p), "bytes": p.stat().st_size}
    write_json(out / "HKCU_P2B_FINAL_MANIFEST.json", manifest)
    if failures: raise SystemExit("P2B_FINAL_BUILD_FAILED:" + "|".join(sorted(set(failures))))
    print(json.dumps(decision, ensure_ascii=False, indent=2, sort_keys=True)); return decision


def main() -> None:
    p = argparse.ArgumentParser(); p.add_argument("--repo-root", default="."); p.add_argument("--output", required=True); args = p.parse_args()
