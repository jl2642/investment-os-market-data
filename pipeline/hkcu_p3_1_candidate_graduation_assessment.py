...
def rebuild_p2b_final(root: Path, out: Path) -> None:
    ...
    subprocess.run(
        [sys.executable, str(root / "pipeline/hkcu_p2b_final_cross_sectional_synthesis.py"),
         "--repo-root", str(root), "--output", str(out)],
        check=True,
    )
    subprocess.run(
        [sys.executable, str(root / "scripts/validate_hkcu_p2b_final_cross_sectional_synthesis.py"),
         "--output", str(out)],
        check=True,
    )
...
def valuation_support(row: pd.Series) -> tuple[str, bool, str]:
    ...
    if all(x is None for x in (ey, pe, dy)):
    ...
    if (ey is not None and ey > 0) or (dy is not None and dy > 0):
        return "SUPPORTIVE", True, f"Accepted valuation context: earnings_yield={ey}, pe_ratio={pe}, dividend_yield_365d={dy}."
    if pe is not None and pe > 0:
        return "LIMITED", True, f"Accepted valuation context is limited rather than absent: earnings_yield={ey}, pe_ratio={pe}, dividend_yield_365d={dy}."
    return "ADVERSE_OR_UNUSABLE", False, f"Accepted valuation fields are present but do not provide positive support: earnings_yield={ey}, pe_ratio={pe}, dividend_yield_365d={dy}."
...
def choose_dimension(grp: pd.DataFrame, directions: set[str]) -> pd.Series | None:
    ...
    return x.sort_values(["_priority", "research_dimension"]).iloc[0]
...
def thesis_package(grp: pd.DataFrame, sec_row: pd.Series) -> tuple[str, str, str, str]:
    ...
    sleeve = str(sec_row.get("primary_sleeve") or "UNSPECIFIED")
    if positive is not None:
        thesis = f"P2A primary sleeve={sleeve}; accepted {positive['research_dimension']} evidence is positive: {positive['final_finding']}."
        strength = "COMPANY_EVIDENCE_SUPPORTED"
    elif mixed is not None:
        thesis = f"P2A primary sleeve={sleeve}; accepted {mixed['research_dimension']} evidence is mixed and requires disciplined monitoring: {mixed['final_finding']}."
        strength = "MIXED_COMPANY_EVIDENCE"
    else:
        first = grp.sort_values("research_dimension").iloc[0]
        thesis = f"P2A primary sleeve={sleeve} remains the quantitative research rationale; company-specific evidence is not positive enough to strengthen it. Current reference finding: {first['final_finding']}."
    ...
    if negative is not None:
        ...
        if not cap.empty:
            r = cap.iloc[0]
            falsifier = f"Principal downside/falsifier: unresolved {r['research_dimension']} uncertainty; required evidence: {str(r.get('next_required_evidence') or '')}."
        else:
            r = grp.iloc[0]
    ...
    triggers: list[str] = []
    for value in grp["monitor_trigger"].astype(str):
        value = value.strip()
        if value and value.lower() not in {"nan", "none"} and value not in triggers:
            triggers.append(value)
    return thesis, falsifier, " | ".join(triggers[:3]), strength


def rule_row(sid: str, code: str, name: str, rank: int, rule: dict[str, Any], state: str, rationale: str) -> dict[str, Any]:
    return {
...
def build(root: Path, out: Path) -> dict[str, Any]:
    ...
    p3_0_path = root / contract["authoritative_inputs"]["p3_0_contract"]
    fmdl5e_path = root / contract["authoritative_inputs"]["fmdl5e_contract"]
    p3_0 = read_json(p3_0_path)
    fmdl5e = read_json(fmdl5e_path)
    ...
    p2b_decision = read_json(upstream / "HKCU_P2B_FINAL_DECISION.json")
    if p2b_decision.get("status") != "PASS_P2B_FINAL_CROSS_SECTIONAL_SYNTHESIS":
        failures.append("P2B_FINAL_NOT_PASS")

    sec = pd.read_csv(upstream / "HKCU_P2B_FINAL_SECURITY_CROSS_SECTION.csv", dtype={"stock_code_5d": str}, keep_default_na=False)
    dim = pd.read_csv(upstream / "HKCU_P2B_FINAL_COMPANY_DIMENSION_SURFACE.csv", dtype={"stock_code_5d": str}, keep_default_na=False)
    hkcu = pd.read_csv(root / contract["authoritative_inputs"]["hkcu_current"], dtype={"stock_code_5d": str}, keep_default_na=False, encoding="utf-8-sig")
    sec["stock_code_5d"] = sec["stock_code_5d"].astype(str).str.zfill(5)
    dim["stock_code_5d"] = dim["stock_code_5d"].astype(str).str.zfill(5)

    if len(sec) != int(entry["entry_security_count"]): failures.append(f"SECURITY_COUNT:{len(sec)}")
    if sec["security_id"].duplicated().any(): failures.append("DUPLICATE_SECURITY")
    if len(dim) != int(entry["entry_security_count"]) * 3: failures.append(f"DIMENSION_ROWS:{len(dim)}")
    if dim.duplicated(["security_id", "research_dimension"]).any(): failures.append("DUPLICATE_DIMENSION")
    if set(dim["research_dimension"]) != set(DIMS): failures.append("DIMENSION_VOCABULARY")

    hkcu_map = hkcu.set_index("security_id", drop=False)
    missing_hkcu = sorted(set(sec["security_id"]) - set(hkcu_map.index))
    if missing_hkcu: failures.append("HKCU_MEMBERSHIP_MISSING:" + ",".join(missing_hkcu))
    ...
    if observed_blocked != blocked_ids: failures.append("BLOCKER_SET_MISMATCH")

    rules = {r["rule_id"]: r for r in p3_0["graduation_rules"]}
    if set(rules) != {f"P3R{i:02d}" for i in range(1, 13)}: failures.append("RULE_ID_SET")
    ...
    for s in sec.sort_values("p2a_overall_rank").itertuples(index=False):
        ...
        eligibility_date = str(h.get("eligibility_as_of_date") or "")
        factor_date = str(h.get("fmdl5e_as_of_date") or "")
        # `market_latest_date` is the accepted current-market timestamp in HKCU Current.
        # P2B Final does not carry a generic `price_date` field, so reading it there would
        # falsely mark all 77 securities stale.
        market_date = str(h.get("market_latest_date") or "")
        factor_fresh, factor_age_days = freshness_within_window(factor_date, contract["as_of_date"], maximum_age_days)
        market_fresh, market_age_days = freshness_within_window(market_date, contract["as_of_date"], maximum_age_days)
        fresh_ok = freshness == "CURRENT" and eligibility_date == contract["as_of_date"] and factor_fresh and market_fresh
        ...
        if ah_pair == "TRUE_AH_PAIR":
            ...
            ah_review = f"Confirmed same-issuer A/H pair reviewed. relative_value_direction={rv_dir}, h_discount_to_a_pct={discount}. Spread is context only and not alpha."
        else:
        ...
        hard_eval = {
            ...
            "P3R03": (publication and buy_eligible and not sell_only and investability_ok,
                        f"publication_eligible={publication}; buy_eligible={buy_eligible}; sell_only={sell_only}; investability_status={investability}."),
            "P3R04": (fresh_ok,
                        f"freshness_status={freshness}; eligibility_as_of_date={eligibility_date}; market_latest_date={market_date}; market_age_days={market_age_days}; fmdl5e_as_of_date={factor_date}; factor_age_days={factor_age_days}; accepted_max_age_days={maximum_age_days}; assessment_as_of={contract['as_of_date']}."),
            ...
            "P3R09": (investability_ok, f"Accepted HKCU/P2A investability remains {investability}; no lower Phase-3 threshold is invented."),
        }
        decision_eval = {
        ...
        hard_pass = all(v[0] for v in hard_eval.values())
        decision_pass = all(v[0] for v in decision_eval.values()) and ah_state in {"PASS", "NOT_APPLICABLE"}
        ...
        if retained:
            ...
            reasons = []
            if not hard_pass: reasons.append("one_or_more_hard_rules_failed")
            if not decision_pass: reasons.append("one_or_more_decision_rules_failed")
            if material_cap_count: reasons.append(f"material_confidence_caps={material_cap_count}")
            proposal_reason = "; ".join(reasons)
        elif valuation_state == "SUPPORTIVE" and bounded_cap_count == 0 and negative_count == 0 and positive_count >= 1:
            ...
            proposal_reason = "All applicable rules pass, but bounded uncertainty, mixed/neutral evidence, event timing or limited valuation support warrants Watch rather than Core."
        ...
        security_rows.append({
            ...
            "price_age_days": market_age_days,
    ...
    if len(assessments) != int(contract["acceptance"]["security_assessment_count"]): failures.append(f"ASSESSMENT_COUNT:{len(assessments)}")
    if len(rule_surface) != int(contract["acceptance"]["rule_assessment_row_count"]): failures.append(f"RULE_ASSESSMENT_COUNT:{len(rule_surface)}")
    if rule_surface.duplicated(["security_id", "rule_id"]).any(): failures.append("DUPLICATE_RULE_ASSESSMENT")
    if not set(rule_surface["rule_state"]).issubset(set(contract["rule_states"])): failures.append("RULE_STATE_VOCABULARY")
    if not set(assessments["proposal_state"]).issubset(set(contract["proposal_states"])): failures.append("PROPOSAL_STATE_VOCABULARY")

    blocked = assessments[assessments["proposal_state"].eq("HOLD_RETAINED_INVESTMENT_BLOCKER")].copy()
    if set(blocked["security_id"]) != blocked_ids: failures.append("P3_1_BLOCKER_SET")
    if len(blocked) != int(entry["retained_blocker_security_count"]): failures.append(f"P3_1_BLOCKER_COUNT:{len(blocked)}")
    if assessments["formal_candidate_graduation"].map(as_bool).any(): failures.append("FORMAL_GRADUATION_OCCURRED")
    if assessments["candidate_pool_mutation"].map(as_bool).any(): failures.append("CANDIDATE_MUTATION_OCCURRED")
    if assessments["alpha_score"].notna().any(): failures.append("ALPHA_SCORE_PRESENT")
    if not assessments["trade_authority"].eq(TRADE_AUTHORITY).all(): failures.append("TRADE_AUTHORITY_MUTATION")
    ...
    assessments.to_csv(assessment_path, index=False)
    rule_surface.to_csv(rule_path, index=False)
    ...
    quality = {
        ...
        "market_date_source": "HKCU_CURRENT_MARKET_LATEST_DATE",
    ...
    for r in assessments.itertuples(index=False):
        report_lines.append(f"| {r.p2a_overall_rank} | {r.stock_code_5d} | {r.security_name} | {r.proposal_state} | {r.valuation_support_state} | {r.material_confidence_cap_count} | {r.bounded_confidence_cap_count} |")
    report_lines += ["", "## Boundary", "", "P3-1 is assessment-only. Proposal states are not formal Candidate Pool membership, portfolio allocation, simulation positions, real-account positions or trade authority.", ""]
    ...
    manifest = {
        ...
        "p3_0_contract_sha256": sha256_file(p3_0_path),
        "fmdl5e_contract_sha256": sha256_file(fmdl5e_path),
